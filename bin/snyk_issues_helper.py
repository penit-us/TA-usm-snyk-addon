...
INPUT_TYPE = "snyk_issues"
SOURCETYPE = "snyk:rest:issues"
DEFAULT_LOOKBACK_DAYS = 7


def extract_items(page: dict):
    """Issues follows standard JSON:API -- data is a flat array of {id, type, attributes}."""
    ...
    for input_name, input_item in inputs.inputs.items():
        ...
        try:
            ...
            for org in orgs:
                ...
                start_url = (
                    f"{sc.SNYK_API_BASE}/rest/orgs/{org_id}/issues"
                    f"?version={version}&updated_after={updated_after}&limit=100"
                )
                    ...
                    for record in sc.paginate(
                        ...
                        event_writer.write_event(
                            smi.Event(
                                ...
                                source=f"snyk://group/{group_id}/org/{org_id}/issues",
                            )
                        )
                        event_count += 1
            ...
            log.log_exception(
                logger, e, "snyk_issues_error",
                msg_before=f"Exception raised while collecting Snyk issues for {normalized_input_name}: "
            )
